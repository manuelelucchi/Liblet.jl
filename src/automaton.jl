...
struct Automaton
    ...
    function Automaton(N::Iterable, T::Iterable, transitions::Iterable, q0::AbstractString, F::Iterable)
        ...
        if ~isempty(N ∩ T) throw(ArgumentError("The set of states and input symbols are not disjoint, but have $(collect(N ∩ T)) in common.")) end
        if q0 ∉ N throw(ArgumentError("The specified q0 ($q0) is not a state.")) end
        if F ⊈ N throw(ArgumentError("The accepting states $(collect(F - N)) in F are not states.")) end
        bad_trans = [t for t ∈ transitions if t.from ∉ N || t.to ∉ N || t.label ∉ (T ∪ Set(["ε"]))]
        if ~isempty(bad_trans) throw(ArgumentError("The following transitions contain states or symbols that are neither states nor input symbols: $(bad_trans)")) end
...
function Automaton(G::Grammar)::Automaton 
    ...
    for P ∈ G.P
        if length(P.right) > 2 throw(ArgumentError("Production $P has more than two symbols on the left-hand side")) end
        if length(P.right) == 2
            A = P.left
            a = P.right[1]
            B = P.right[2]
            if ~(a ∈ G.T && B ∈ G.N) throw(ArgumentError("Production $P right-hand side is not of the aB form")) end
